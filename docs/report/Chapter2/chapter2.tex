...
changed to \texttt{npm test}.



% - Angular front test support
% - adding the extra Dockerfile.test to run the tests. The Dockerfile could not be used as it's a production ready
% file. npm does not exists as files are compiled so Karma tests could not be started.
% - PhantomJS headless browser is started and runs tests
% - Components UI is not tested
% - Componentes private methods are being tested
% - Offline services are being unit tested
% - HTTP services are not being tested, as the endpoint would have to be mocked and
% there was a plan of adding end-to-end selenium tests which would test both
% the UI and the HTTP services at the same time. Given the time contraints the choice was made to
% go with extra selenium tests, but angular unit tests should certainly be extended if the
% application lifespan exceeded the final project submission.
