...
For the complete, final design see \autoref{chap:finaldesign} of this report.
...
the tool. Students could join an ongoing lecture using a session key provided by the
lecturer. They could then participate in the lecture by watching the slides, and
submitting answers to quizzes as they come in. The correct answer could be presented
back to them in a form of a bar chart.
...
\autoref{fig:burn1} shows how 14 issues worth 16 story points have been implemented over the course of the sprint.
...
valid session cookie with a JWT (JSON Web Token)\cite{44} present as described in the subsection below, before allowing
...
including internal object ids. Working with JSON objects is very easy in JavaScript applications,
and because MongoDB has a wider support than traditional database engines in the Node.js
community, MongoDB has been chosen over a relational database.
Mongoose\cite{48} is a popular MongoDB object modelling library which has
...
PDFs for each slide on disk. These PDFs would be then converted to PNG(Portable Network Graphics) images using the scissors\cite{53}
...
\newpage
\section{Sprint 4 - Fancy Quizzes and Defect Fixing}
\subsection{Sprint Planning}
...
Multicontainer Elastic Beanstalk environment, any data stored in the MongoDB
...
Every time a new lecture broadcast is
...
directory is listed in \autoref{tab:routes}. An example of a controller implementation has
been included in the \autoref{chap:codesamples} of this report as the \autoref{sample:controller}.
...
has been performed by showing progress to the client on weekly basis, and running a
real life lecture with the end users, once the first version of the
...
is also overridden to \texttt{npm test} to start the mocha tests.
...
can be marked as quizzes, lectures be removed, sessions associated with lectures and reports
...
show the progress, and to ask for feedback. This process could be thought of as a form
...
approximately 20 users at once. Lecture started by explaining to students,
...
The one hour lecture ended with a voluntary user evaluation, where students
were asked to provide their feedback on their experience of using the tool,
and their thoughts on how it compared to Qwizdom.

The session report generated
...
Students were overall pleased with the Quiz Tool and liked the user interface created.
A significant majority preferred it to Qwizdom, although 14.3\% people surveyed
mentioned displaying of correct answers on their machines was not clear.
It also became apparent that once the lecturer starts a broadcast, a full screen
mode should be added, to take the advantage of the entire screen while showing
lecture slides using a projector. Currently, the navigation buttons and the navbar
make the actual slide smaller, and therefore likely illegible for people sitting
far back in a lecture hall. Having a full screen mode would also prevent
students from being able to see how others are answering in real time,
and not thinking about the correct answer on their own. Finally, the tool
proved to be noticeably faster than Qwizdom.
