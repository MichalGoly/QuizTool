...
\newpage
There were three containers in the system:
\begin{itemize}
  \item \textbf{client} - which consisted of the Angular front end, and an nginx reverse proxy
  serving web pages to students and lecturers. It contained the Socket.io client to emit messages
  to other clients via the Socket.io engine running on the back end.
  \item \textbf{server\_node} - which contained the back end code written in Express. It also
  contained the Socket.io engine allowing the chat application to push messages to clients
  using the WebSocket Protocol.
  \item \textbf{database} - which contained the MongoDB instance.
\end{itemize}
